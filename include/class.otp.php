...
class otp {
	...
	public function initializeOtp($userInput = false, $seed = false, $sequenceCount = false, $algorithm = false) {
		...
		if($this->isValidSequence($sequenceCount) !== true) return false;
		if(!in_array($algorithm, $this->availableAlgorithms)) return false;
	...
	public function authAgainstHexOtp($userInput = false, $masterHexOtp = false, $masterHexOtpType = false, $sequence = false, $algorithm = false) {
		...
		if($algorithm === false || !in_array($algorithm, $this->availableAlgorithms)) return $output;
	...
	public function setAlternativeDictionary($dictionary) {
		if($this->isValidDictionary($dictionary)) return false;
	...
	public function binaryOtp($passString = false, $algorithm = false) {
		...
		if($algorithm === false || $this->isValidAlgorithm($algorithm) !== true) return false;
		if(!in_array($algorithm, $this->availableAlgorithms)) return false;
	...
	/**
	 * Creates a challenge string that adhers to the specification
	 *
	 * @param		string	seed
	...
	public function isWordsOtp($wordsOtp = false) {
		if($wordsOtp === false || !is_string($wordsOtp)) return false;

		if(preg_match('/^\s*[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s+[a-zA-Z]{1,4}\s*$/', $wordsOtp)) return true;
		else return false;
	}
	...
	public function isBitOtp($bitOtp = false) {
		if($bitOtp === false || !is_string($bitOtp)) return false;

		if(preg_match('/^\s*[01]{64}\s*$/', $bitOtp)) return true;
		else return false;
	}
	...
	public function isDecOtp($decOtp = false) {
		if($decOtp === false || !is_string($decOtp)) return false;

		if(preg_match('/^\s*[0-9]{1,4}\s+[0-9]{1,4}\s+[0-9]{1,4}\s+[0-9]{1,4}\s+[0-9]{1,4}\s+[0-9]{1,4}\s*$/', $decOtp)) return true;
		else return false;
	}
	...
	public function isValidChallenge($challenge = false) {
		...
		if(preg_match('/^otp-(md4|md5|sha1)\s+[0-9]+\s+[0-9a-z]+\s+$/', $challenge)) return true;
		else return false;
	}


	/**
	 * Tests whether the supplied pass-phrase adheres to the specification
	 *
	...
	/**
	 * Tests whether the supplied seed adheres to the specification
	 *
	 * @param		string	seed
	...
	public function isValidSeed($seed = false) {
		...
		elseif(!preg_match('/^[0-9a-z]{1,16}$/', $seed)) return false;
		else return true;
	}
	...
	public function isValidSequence($sequence = false) {
		...
		elseif(!preg_match('/^[0-9]+$/', $sequence)) return false;
		else return true;
	}


	/**
	 * Tests whether the supplied algorithm is acceptable
	 *
	 * @param		string	seed
	 * @return	bolean	TRUE if the seed is valid, otherwise FALSE
	 */
	public function isValidAlgorithm($algorithm = false) {
		if(strcasecmp($algorithm, 'md4') == 0
			|| strcasecmp($algorithm, 'md5') == 0
			|| strcasecmp($algorithm, 'sha1') == 0) return true;
		else return false;
	}


	/**
	 * Test whether the supplied data represent a valid dictionary
	 *
