...
oldSeeds      - a list of seeds that were used in the past and should be
                avoided.
sequenceCount - number of iterations = generated one-time passwords.
algorithm     - chosen hash algorithm.
...
7.6 setAlternativeDictionary(array $dictionary)
...
7.7 generateSeed([int $minLength[, int $maxLength[, array $excludedSeeds]]])
...
7.8 createChallenge(string $seed, int $sequence, string $algorithm)
...
7.9 parseChallenge(string $challenge)
...
7.10 reformatHexOtp(string $hexOtp[, string $letterCase[, string $format]])
