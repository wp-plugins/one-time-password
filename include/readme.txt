...
7. Description of main methods
	 7.1 initializeOtp()
	 7.2 reinitializeOtp()
	 7.3 authAgainstHexOtp()
	 7.4 generateOtp()
	 7.5 generateOtpList()
	 7.6 setCustomDictionary()
	 7.7 generateSeed()
	 7.8 createChallenge()
	 7.9 parseChallenge()
	 7.10 reformatHexOtp()
	 7.11 getAvailableAlgorithms()
...
number) for which they must compute a response based on the knowledge of
the secret pass-phrase. They are supposed to use external devices (e.g. a mobile
phone) equipped with appropriate software (e.g. a Java applet) that would
compute an one-time password after inputting a server challenge and the secret
pass-phrase.
...
challenge sequence number (N-1) an used hash algorithm.
...
STEP 5: The user starts an OTP software in his mobile phone, chooses the right
...
a 16 character long hexadecimal value, whichever is more convenient for him.

STEP 6: The user enters the generated one-time password into a form presented
by the system.
...
* VeJOTP - a Java midlet that can run on very basic J2ME implementations.
	Author: Veghead
	URL: http://fatsquirrel.org/software/vejotp/

* J2ME OTP - an one-time password generator for mobile phones.
	Author: Marco Ratto
	URL: http://otp-j2me.sourceforge.net/

* j2me-otp - another J2ME application that can be used on mobile phones.
...
The class provides generic methods needed for creation of one-time password
authentication as describe in RFC 2289 and as such can be used both for
...
1. While having the user signed to your system via secure environment, provide
...
Obviously, you my implement the class differently but then you must take into
account security issues that might arise of your implementation.
...
The use of the one-time password system requires perform two operations:
...
* Reinitialization - as the number of OTPs "created" during initialization is
	...
	an user will run out of available OTPs and creation of new sequence is needed.
...
7.1 initializeOtp($userInput, $seed, $sequenceCount, $algorithm)
...
7.2 reinitializeOtp($userInput, $newSeed, $oldSeed, $sequenceCount, $algorithm)
...
* oldSeed				- seed that was used in the previous OTP sequence.
* sequenceCount	- number of iterations = generated one-time passwords.
* algorithm			- chosen hash algorithm.
...
7.3 authAgainstHexOtp($userInput, $masterHexOtp, $masterHexOtpType,
		$sequence, $algorithm)
...
7.4 generateOtp(string $passPhrase, string $seed, integer $sequence
		[, string $algorithm])
...
(
    ...
    // one-time passwords for sequence 98-1
...
7.6 setAlternativeDictionary($dictionary)
...
7.7 generateSeed([$minLength[, $maxLength[, $excludedSeeds]]])
...
Return value:
A string which is a seed conforming with RFC 2289.


7.8 createChallenge($seed, $sequence, $algorithm)

Description:
Creates a challenge string that has a standard syntax which can be recognized
by automated generators.
...
Return value:
A challenge string which is conforming with RFC 2289.
...
Return value:
An array of challenge elem4nts:
'algorithm' - 'md4', 'md5' or 'sha1'.
'sequence'  - sequence number.
'seed'      - challenge seed.


7.10 reformatHexOtp($hexOtp[, $letterCase[, $format]])
...
* Human stupidity - it's nice that an user authenticates with an one-time
...
* Before implementing the class, you should be familiar with RFC 2289. Reading
	this "readme.txt" is not enough.

* Number of iterations (sequence count) - you should limit the user to choose
	a reasonable sequence count. Default value is 100. 1000 is alright but
	processing of 100000 may take quite some time during generation.
